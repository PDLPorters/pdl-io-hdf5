...
sub set {
  my ($self, $pdl) = @_;
  my %options = @_ > 2 ? @_[2..$#_] : ();

  my ($parent, $datasetID, $name) = @$self{qw(parent ID name)};
  my $groupID = $parent->IDget;
  ...
  if ($pdl->isa('PDL::Char')) { #  Special Case for PDL::Char Objects (fixed length strings)
    ...
    PDL::IO::HDF5::H5Tset_size($internalhdf5_type, $length ); # make legth of type equal to strings
    ...
    unless (defined($internalhdf5_type = $PDLtoHDF5internalTypeMapping{$type})) {
      carp "Error Calling ".__PACKAGE__."::set: Can't map PDL type to HDF5 datatype\n";
      return undef;
    }
    unless (defined($hdf5Filetype = $PDLtoHDF5fileMapping{$type})) {
      carp "Error Calling ".__PACKAGE__."::set: Can't map PDL type to HDF5 datatype\n";
      return undef;
    }
  ...
  my $udims = my $dims = PDL::IO::HDF5::packList(@dims);
  ...
  if ($datasetID == 0) {  # Dataset not created yet
    ...
    if (exists($options{unlimited})) {
      ...
      # /* Create the dataset. */
      $datasetID = PDL::IO::HDF5::H5Dcreate($groupID, $name, $hdf5Filetype,
	$dataspaceID, $propertiesID);
    } else {
      # /* Create the dataset. */
      $datasetID = PDL::IO::HDF5::H5Dcreate($groupID, $name, $hdf5Filetype,
	$dataspaceID, PDL::IO::HDF5::H5P_DEFAULT());
    }
    if ($datasetID < 0) {
